...
def sample_and_group(npoint, radius, nsample, xyz, points, knn, csa_p):
    ...
    S = npoint
    fps_idx = farthest_point_sample(xyz.contiguous(), npoint)  # [B, npoint, C] # 通过FPS算法找到质心索引
...
class PointNetFeaturePropagation(nn.Module):
    ...
    def forward(self, xyz1, xyz2, points1, points2):
        ...
        new_points = torch.cat((interpolated_points.permute(0, 2, 1) ,  points1),dim=1)
...
class get_model(nn.Module):
    def __init__(self, num_class, normal_channel=True):
        ...
        self.k = 16  # 邻居数量
        self.total_points = 4096
        self.actiavte_function = nn.LeakyReLU(0.3, inplace=True)
        self.normal_channel = normal_channel

        self.first_f = nn.Sequential(nn.Conv1d(3+3, 32, 1, bias=False), nn.BatchNorm1d(32), self.actiavte_function
        ...
        self.cat = nn.Sequential(
            ...
            nn.Conv1d(2048,512, 1, bias=False), nn.BatchNorm1d(512), self.actiavte_function)
        # # self.seg_head = nn.Sequential(_PSPModule(1024,bin_sizes=self.bin_size,activate_function=self.actiavte_function),)

        self.up_sample1 = PointNetFeaturePropagation(in_channel=512+512, mlp=[1024,512], last=512 * 2,
                                                     activate_function=self.actiavte_function,
                                                     up_npoint=self.total_points // 64)  # 64  - 16
        self.up_sample2 = PointNetFeaturePropagation(in_channel=512+256, mlp=[512+256,256], last=256 * 2,
                                                     activate_function=self.actiavte_function,
                                                     up_npoint=self.total_points // 16)  # 16 - 4
        self.up_sample3 = PointNetFeaturePropagation(in_channel=256+128, mlp=[256+128,128], last=128 * 2,
                                                     activate_function=self.actiavte_function,
                                                     up_npoint=self.total_points // 4)  # 4 -2
        self.up_sample4 = PointNetFeaturePropagation(in_channel=128+64, mlp=[128+64,128], last=64 * 2,
                                                     activate_function=self.actiavte_function,
                                                     up_npoint=self.total_points)

        self.msa0 = nn.Sequential(nn.Conv1d(64+6+3,(64+6),1,bias=False),nn.BatchNorm1d((64+6)),self.actiavte_function,
                                 nn.Conv1d((64+6), 64, 1, bias=False), nn.BatchNorm1d(64), self.actiavte_function,)

        self.msa1 = nn.Sequential(nn.Conv1d(64*2+128*2,(64*2+128*2),1,bias=False),nn.BatchNorm1d((64+128)*2),self.actiavte_function,
                                 nn.Conv1d((64+128)*2, 128, 1, bias=False), nn.BatchNorm1d(128), self.actiavte_function,)


        self.msa2 = nn.Sequential(nn.Conv1d(128+256*2, (128+256*2), 1, bias=False), nn.BatchNorm1d((128+256*2)),self.actiavte_function,
                                nn.Conv1d((128+256*2) , 256, 1, bias=False), nn.BatchNorm1d(256),self.actiavte_function,)

        self.msa3 = nn.Sequential(nn.Conv1d(256 + 512*2, (256 + 512*2), 1, bias=False), nn.BatchNorm1d((256 + 512*2)), self.actiavte_function,
                                  nn.Conv1d((256 + 512*2), 512, 1, bias=False), nn.BatchNorm1d(512),self.actiavte_function, )

        self.last = nn.Sequential(nn.Dropout(0.2), nn.Conv1d(128, num_class, 1, ))
    def forward(self, xyz, trans=None):
       ...
       # print(xyz.shape,norm.shape,up_featrue.shape,up_position.shape)
        msa0 = self.msa0(torch.cat([xyz,norm,up_position+up_featrue],dim=1))
